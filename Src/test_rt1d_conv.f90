...
!  using subroutine rt1d_conv() where RTM with conventional implementation
!
!  Input
...
!  Usage
!
!       test_rt1d_conv.x < in.H adj=[0/1] vel=vel.H snap=[0/1] snapout=snap.H > out.H
!
!% ----------------
program zyxabc
implicit none
...
call initpar()
call doc('./../Src/test_rt1d_conv.f90')
...
call rt1d_conv( adj, dt,dz, nt,nz, modl, data, v, snap)
if (adj.eq.0) then
