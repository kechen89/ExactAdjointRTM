...
!
subroutine rt1dinv( nz,model, nt,data, dt,dz,vel, res, niter)
integer         nz,nt, niter,iz,it,iter
real           model(nz),data(nt), dt,dz,vel(nz),res(niter+1), dot
real  dmodel(nz), smodel(nz), dr(nt), sr(nt)
real  rr(nt)
do iz= 1, nz
  model(iz) = 0.0
end do 
...
do iter = 0, niter  
  call rt1d( 1, dt,dz, nt,nz, dmodel, rr, vel, 0)
  call rt1d( 0, dt,dz, nt,nz, dmodel, dr, vel, 0)
